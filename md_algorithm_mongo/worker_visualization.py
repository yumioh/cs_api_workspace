...
plt.scatter(arranged_accident.index, arranged_accident, color="red", label="사고",s=8)
plt.scatter(arranged_non_accident.index, arranged_non_accident, color="dodgerblue", alpha=0.4, label="비사고", s=8)
...
plt.scatter(arranged_accident_log.index, arranged_accident_log, color="red", label="사고",s=8)
plt.scatter(arranged_non_accident_log.index, arranged_non_accident_log, color="dodgerblue", alpha=0.4, label="비사고", s=8)
...
print("-------------------사고 vs 비사고 log(MD) 정규화 산점도--------------------")  
accident_log_normalized = pd.read_csv("./md_algorithm_mongo/data/accident_log_normalized.csv")
non_accident_log_normalized = pd.read_csv("./md_algorithm_mongo/data/non_accident_log_normalized.csv")

print(accident_log_normalized.head())
print(non_accident_log_normalized.head())

arranged_accident_log_nor = accident_log_normalized["normalized"].sort_values().reset_index(drop=True)
arranged_non_accident_log_nor = non_accident_log_normalized["normalized"].sort_values().reset_index(drop=True)

plt.scatter(arranged_accident_log_nor.index, arranged_accident_log_nor, color="red", label="사고",s=8)
plt.scatter(arranged_non_accident_log_nor.index, arranged_non_accident_log_nor, color="dodgerblue", alpha=0.4, label="비사고", s=8)
plt.title(f"사고 vs 비사고 log(MD) 정규화값 산점도",fontdict={'weight': 'bold', 'size' : "20"})
plt.xlabel("Index")
plt.ylabel("MD")
plt.grid(True)
plt.legend()
